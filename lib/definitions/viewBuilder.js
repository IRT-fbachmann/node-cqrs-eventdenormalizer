...
function ViewBuilder(meta, denormFn) {
  ...
  //  i.e. function expects only two arguments,
  //  either a promise must be returned
  //  or function is handled synchronously
  if (denormFn.length === 2) {
    this.denormFn = function (evtData, vm, clb) {
      // Try-catch to catch thrown error if fn is syncronous
      try {
        var fnResult = /** @type {Promise|any} */ (denormFn.call(this, evtData, vm));
  ...
  this.denormFn = function (evtData, vm, clb) {
    ...
    try {
      console.log('Handle event', evtData)
